document.addEventListener('DOMContentLoaded', () => {
    ...
    async function apiCall(endpoint, method = 'GET', data = null) {
        const url = `http://localhost:5000${endpoint}`; // Mock API; update to bonepaint.com later
        const options = {
            method,
    ...
    pastGenerationsBtn.addEventListener('click', async () => {
        ...
        if (response && response.length > 0) {
            ...
            if (response.length === 3) {
                ...
                moreBtn.className = "bonepaint-button";
    ...
    generateVideoBtn.addEventListener('click', async () => {
        ...
        const creditsResponse = await apiCall(`/api/credits?wallet=${wallet}`);
